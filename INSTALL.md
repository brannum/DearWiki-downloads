# Install DearWiki 1.0.4 on Linux
...
Download an installer and `SHA256SUMS` from the [same release](https://github.com/brannum/DearWiki-downloads/releases/tag/v1.0.4). Keep them in the same directory.
...
```sh
chmod +x DearWiki-1.0.4-linux-x86_64.AppImage
./DearWiki-1.0.4-linux-x86_64.AppImage
```
...
```sh
sudo apt install ./DearWiki-1.0.4-linux-amd64.deb
```

Then open **DearWiki** from your application menu. Run the application as your normal user, not with `sudo`.

## Snap Store

Install the stable release from the [Snap Store](https://snapcraft.io/dearwiki):

```sh
sudo snap install dearwiki
```

Launch DearWiki from your application menu as your normal user. Optional removable-media access is disconnected by default; connect it only if you want DearWiki to access files on external drives:
...
Before an update, export a backup from DearWiki and keep it outside the application's installation folder.
...
- **Snap:** keep Store updates enabled. To request an update, use `sudo snap refresh dearwiki`.
